...
$(document).ready(function(){
	...
	//$('#ajax_list').load('?c=Coupon&a=ajax_list?pageNum=<?php echo ($currentPage); ?>');
	$("#all").click(function(){ 
            ...
            <div class="content" style="">
            	<div class="wrapper page">
                    ...
                    <div class="pane">
                    	...
                    	<div class="toolbar"><a class="btn" href="javascript:void(0)"><span class="icon icon-refresh"></span></a><a class="btn" href="?c=Coupon&a=add"><span class="icon icon-run"></span><span class="text">新建</span></a><a class="btn" href="javascript:void(0)" onclick="del()"><span class="text">删除</span></a>
                            <!--
						...
						<div class="select-con">
							...
							有效时间：<input type="text1" style="width:100px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px" name="valid_start" id="valid_start" class="Wdate" onfocus="WdatePicker()" value="<?php echo ($valid_start); ?>"/>至<input type="text1" style="width:100px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px" name="valid_end" id="valid_end" class="Wdate" onfocus="WdatePicker()" value="<?php echo ($valid_end); ?>"/>
							<div class="form-search"><input type="search" id="keywords" placeholder="优惠券ID/优惠券名称" value="<?php echo ($keywords); ?>"></div>
						...
						<tbody>
						<?php
 $typearr[0]='满减券'; $typearr[1]='满赠券'; $typearr[2]='代金券'; $typearr[3]='折扣券'; $statusarr[0]='待发布'; $statusarr[1]='上线'; $statusarr[2]='下线'; $statusarr[3]='已结束'; ?>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $shop=M('Shop')->where(array('member_id'=>$vo['member_id']))->find(); ?>
						<tr>
						<td class="checkbox"><input type="checkbox" name="id[]" value="<?php echo ($vo["id"]); ?>"></td>
						<td><?php echo ($vo["id"]); ?></td>
						<td><?php echo ($vo["title"]); ?></td>
						<td><?php echo ($vo["draw_num"]); ?></td>
						<td><?php echo ($shop["title"]); ?><br /><?php echo ($shop["id"]); ?></td>
						<td><?php echo ($typearr[$vo['type']]); ?></td>
						<td><?php echo ($statusarr[$vo['status']]); ?></td>
						<td><?php echo ($vo["valid_start"]); ?>至<?php echo ($vo["valid_end"]); ?></td>
						<td><?php echo date('Y-m-d H:i',$vo['addtime']);?></td>
						<td><a href='?c=Coupon&a=edit&id=<?php echo ($vo["id"]); ?>'>修改</a> 
						<?php
 if($vo['status']==0 || $vo['status']==2) { ?>
						<a href='?c=Coupon&a=online&id=<?php echo ($vo["id"]); ?>'>上线</a> 
						<?php
 } if($vo['status']==1) { ?>
						<a href='?c=Coupon&a=offline&id=<?php echo ($vo["id"]); ?>'>下线</a>
						<?php
 } ?>
						<a href='?c=Coupon&a=copyadd&id=<?php echo ($vo["id"]); ?>'>复制创建</a></td>
