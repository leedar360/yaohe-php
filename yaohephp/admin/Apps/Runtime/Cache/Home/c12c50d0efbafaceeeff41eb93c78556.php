...
$(document).ready(function(){
	...
	<?php
 if($vo['province_id']>0) { echo "$('#province_id').val(".$vo['province_id'].");\n"; } if($vo['city_id']>0) { echo "$('#city_id').val(".$vo['city_id'].");\n"; } if($vo['area_id']>0) { echo "$('#area_id').val(".$vo['area_id'].");\n"; } if($vo['district_id']>0) { echo "$('#district_id').val(".$vo['district_id'].");\n"; } if($vo['one_id']>0) { echo "$('#one_id').val(".$vo['one_id'].");\n"; } if($vo['industry_class_id']>0) { echo "$('#industry_class_id').val(".$vo['industry_class_id'].");\n"; } if($vo['type']<5) { echo "$('#content_id').val(".$vo['content_id'].");\n"; echo "$('#id_status').val(1);\n"; echo "$('#content_id_html').show();\n"; echo "$('#link_url_html').hide();\n"; } else { echo "$('#id_status').val(0);\n"; echo "$('#link_url').val('".$vo['link_url']."');\n"; echo "$('#content_id_html').hide();\n"; echo "$('#link_url_html').show();\n"; } ?>
 ...
 ?>
            ...
            <div class="content" style="">
            	<div class="wrapper page">
                    ...
                    <div class="pane">
                    <ul class="page-tabs"><li class="selected"><a href="?c=Rotate&a=edit&id=<?php echo ($vo["id"]); ?>" data-permalink="">编辑轮换图</a></li></ul>
                    	<form name="form1" method="post" action="?c=Rotate&a=update" enctype="multipart/form-data" onSubmit="return insert()">
                    	<table width="100%" class="table table-bordered table-hover">
							<thead>
								<tr>
								  <th width="127">所在城市<input type="hidden" name="id" value="<?php echo ($vo["id"]); ?>" /></th>
