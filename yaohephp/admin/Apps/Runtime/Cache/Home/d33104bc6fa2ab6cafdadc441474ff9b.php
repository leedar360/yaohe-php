...
$(document).ready(function(){
	...
	$('li .item').removeClass("selected");
	$('.has-items').eq(5).children(".level-2").children('li').eq(0).addClass('selected');
	$('#ajax_list').load('/index.php/Home/Province/ajax_list?pageNum=<?php echo ($currentPage); ?>');
});
...
function add_province()
{
	...
	{
		$.post("/index.php/Home/Province/insert", { title: $('#title').val() },
			function(data){
				//alert(data.msg);
				$('#ajax_list').load('/index.php/Home/Province/ajax_list?pageNum=<?php echo ($currentPage); ?>');
...
function update_province()
{
	...
	{
		$.post("/index.php/Home/Province/update", { id:$('#edit_id').val(),title: $('#edit_title').val() },
			function(data){
				//alert(data.msg);
				$('#ajax_list').load('/index.php/Home/Province/ajax_list?pageNum=<?php echo ($currentPage); ?>');
...
function del()
{
	...
	$.ajax({
		url: "/index.php/Home/Province/delete",
		type: "post",
		data: variable,
...
<!--<div id="fullbg"></div>-->
<!--弹出窗口-->
<div class="modal" id="modal" style="width: 600px; height: auto; margin-left: -300px; margin-top: -200px; top: 208px; z-index: 1000; left: 683px;display:none"><div class="modal-header" style="cursor: move;"><h4>创建省份<a href="javascript:void(0)" onclick="close_modal()" class="close"><span class="icon-close icon-Large"></span></a></h4></div><div class="modal-content" id=""><form class="form form-horizontal"><fieldset><legend>创建省份</legend><div class="item"><div class="control-label">名称</div><div class="controls"><input type="text" name="title" id="title" autofocus><span id="ajax_txt"></span></div></div>

<div class="form-actions"><input class="btn btn-primary" type="button" onclick="add_province()" value="提交"><input class="btn btn-cancel" type="button" value="取消" onclick="close_modal()"></div></fieldset></form></div><div class="modal-footer"></div></div>
<!--弹出窗口-->


<!--弹出窗口-->
<div class="modal" id="edit_modal" style="width: 600px; height: auto; margin-left: -300px; margin-top: -200px; top: 208px; z-index: 1000; left: 683px;display:none"><div class="modal-header" style="cursor: move;"><h4>编辑省份<a href="javascript:void(0)" onclick="close_edit_modal()" class="close"><span class="icon-close icon-Large"></span></a></h4></div><div class="modal-content" id=""><form class="form form-horizontal"><fieldset><legend>编辑省份</legend><div class="item"><div class="control-label">名称</div><div class="controls"><input type="text" name="edit_title" id="edit_title" autofocus><input type="hidden" name="edit_id" id="edit_id" value="0" /><span id="edit_ajax_txt"></span></div></div>

<div class="form-actions"><input class="btn btn-primary" type="button" onclick="update_province()" value="提交"><input class="btn btn-cancel" type="button" value="取消" onclick="close_edit_modal()"></div></fieldset></form></div><div class="modal-footer"></div></div>
<!--弹出窗口-->
