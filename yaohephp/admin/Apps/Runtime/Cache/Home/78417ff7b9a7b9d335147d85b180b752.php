...
$(document).ready(function(){
	...
	$('#ajax_list').load('?c=City&a=ajax_list&province_id=<?php echo ($province_id); ?>&pageNum=<?php echo ($currentPage); ?>');
	$('#province_id').change(function(){
		//alert('d');
		window.location.replace('?c=City&province_id='+$(this).val()+'&pageNum=<?php echo ($currentPage); ?>');
		//$('#ajax_list').load('?c=City&a=ajax_list?province_id='+$(this).val()+'&pageNum=<?php echo ($currentPage); ?>');
	});
	<?php
...
function update()
{
	...
	else
	{
		$.post("?c=City&a=update", { id:$('#edit_id').val(),title: $('#edit_title').val(),p_id:$('#edit_p_id').val(),is_hot:$('input[name="edit_is_hot"]:checked ').val(),order_num:$('#edit_order_num').val() },
			function(data){
				//alert(data.msg);
				$('#ajax_list').load('?c=City&a=ajax_list&pageNum=<?php echo ($currentPage); ?>');
            ...
            <div class="content" style="">
            	<div class="wrapper page">
                    ...
                    <div class="pane">
                    	...
                    	<div class="toolbar"><a class="btn" href="javascript:void(0)"><span class="icon icon-refresh"></span></a><a class="btn" href="javascript:void(0)" onclick="open_modal()"><span class="icon icon-run"></span><span class="text">新建</span></a><a class="btn" href="javascript:void(0)" onclick="del()"><span class="text">删除</span></a>省份：<div class="select-con"><select class="dropdown-select" name="province_id" id="province_id"><option value="0">请选择</option><?php if(is_array($province_list)): $i = 0; $__LIST__ = $province_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><option value="<?php echo ($item["id"]); ?>"><?php echo ($item["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?></select></div>
                            <!--
