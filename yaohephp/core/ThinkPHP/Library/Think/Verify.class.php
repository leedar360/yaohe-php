...
class Verify {
    ...
    public function check($code, $id = '') {
		...
		//或者:
		$options = array('host' => C('MEMCACHE_HOST'), 'port' => C('MEMCACHE_PORT'), 'timeout' => 1800,
		'persistent' => false);
		$Cache = Cache::getInstance('memcache', $options);
        $key = $this->authcode($this->seKey).$id;
		if($this->authcode(strtoupper($code))!=$Cache->get($key))
		{
			return false;
		}
		return true;
    ...
    public function entry($id = '') {
		...
		'persistent' => false);
		$Cache = Cache::getInstance('memcache', $options);
		$Cache->set($key,$code); // 缓存name数据
		//echo $Cache->get($key);exit;
